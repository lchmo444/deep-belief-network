        ...
        for iteration in range(1, self.n_epochs + 1):
            ...
            if self.early_stop:
                if checkpoint < error:
                    ...
                    checkpoint = error
                    W, b, c = sess.run([self.W, self.b, self.c])
                    if previous_error - error < self.delta_rbm:
                        step += 1
                    else:
                        step = 0
                if step >= self.patience_rbm:
                    break

            if self.verbose:
                print((">> Epoch %d finished \tRBM Reconstruction error %f" +
                      "\n     Patience %d \tDelta %f \tStep %d \tCheckpoint %.6f")
                      % (iteration, error, self.patience_rbm, self.delta_rbm, step, checkpoint))
            previous_error = error

        self.restore_W = tf.assign(self.W, W)
        self.restore_b = tf.assign(self.b, b)
        self.restore_c = tf.assign(self.c, c)
        sess.run([self.restore_W, self.restore_b, self.restore_c])
        ...
        for iteration in range(self.n_iter_backprop):
            ...
            if self.early_stop:
                if checkpoint < error:
                    ...
                    if previous_error - error < self.delta_dbn:
                        step += 1
                    else:
                        step = 0
                if step >= self.patience_dbn:
                    break
            if self.verbose:
                print((">> Epoch %d finished \tANN training loss %f" +
                      "\n     Patience %d \tDelta %f \tStep %d \tCheckpoint %.6f")
                      % (iteration, error, self.patience_dbn, self.delta_dbn, step, checkpoint))
            previous_error = error    
                
        self.restore_W = tf.assign(self.W, W)
        self.restore_b = tf.assign(self.b, b)
        sess.run([self.restore_W, self.restore_b])
